...
class AdversarialLoss(nn.Module):
    ...
    def generator_loss(self, inputs: dict, predictions: dict, jcu: JCU):
        ...
        if self.fm_loss:
            ...
            _, _, gt_feature_maps = jcu(
                mel_targets.transpose(1, 2), emotion_embedding
            )
            fm_loss = self._feature_matching_loss(
                gen_feature_maps, gt_feature_maps
            )
        else:
            fm_loss = torch.zeros(1, requires_grad=False).to(
                gen_conditional_out.device
            )

        generator_loss = 0.5 * (
            self.mse_loss(
                gen_conditional_out, torch.ones_like(gen_conditional_out)
            )
            + self.mse_loss(
    ...
    def discriminator_loss(self, inputs: dict, predictions: dict, jcu: JCU):
        ...
        discriminator_loss_real = 0.5 * (
            self.mse_loss(
                gt_conditional_out, torch.ones_like(gt_conditional_out)
            )
            + self.mse_loss(
                gt_unconditional_out, torch.ones_like(gt_unconditional_out)
            )
        )
